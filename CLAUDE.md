...
This is a Spring Boot service for storing and retrieving recipes, currently early-stage: persistence is in-memory only (see below). See `README.md` for the intended system/database diagrams (`assets/SystemDiagram.png`, `assets/DatabaseDesign.png`).

Package layout under `com.maze.recipe`:
- `controller.RecipeController` — `@RestController` mapped to `/recipes`, exposing `GET /recipes`, `GET /recipes/{id}`, `POST /recipes`, and `DELETE /recipes/{id}`. Delegates directly to `RecipeService`.
...
- `service.RecipeService` — the only entry point into business logic (create/read/delete recipes). It re-validates the mapped `Recipe` entity itself via injected `jakarta.validation.Validator` rather than relying solely on annotation-driven validation at the controller boundary. All ID lookups go through `validateId`, which requires `id > 0` and throws `InvalidIdException` otherwise.
- `exception` — `InvalidIdException`, `InvalidQuantityException`, `RecipeNotFoundException`, plus `GlobalExceptionHandler` (a `@RestControllerAdvice`) which maps `RecipeNotFoundException` to 404 and `InvalidIdException`/`InvalidQuantityException`/`ConstraintViolationException` to 400.
